...
#
"""
<plugin key="tplinksmartplugv2" name="TP-Link Wi-Fi Smart Plug HS100/HS110 v2" version="0.1.0">
    <description>
        <h2>TP-Link Wi-Fi Smart Plug v2</h2>
...
class TpLinkSmartPlugPlugin:
    ...
    def onStart(self):
        ...
        if Parameters["Mode1"] == "HS110" and len(Devices) <= 1:
            ...
            Domoticz.Device(Name="Mesure (A)", Unit=2, Type=243, Subtype=23).Create()
            Domoticz.Device(Name="Mesure (V)", Unit=3, Type=243, Subtype=8).Create()
            Domoticz.Device(Name="Mesure (W)", Unit=4, Type=243, Subtype=31, Image=1, Used=1).Create()
